# Package index

## Data preparation

- [`prepare_network_data()`](https://localpolitics.github.io/lpanda/reference/prepare_network_data.md)
  : Prepare Network Data for LPANDA

## Visualization

- [`plot_continuity()`](https://localpolitics.github.io/lpanda/reference/plot_continuity.md)
  : Visualization of Candidacy Continuity Diagram

## Datasets

### Municipalities

- [`Bublava_SO_cz`](https://localpolitics.github.io/lpanda/reference/Bublava_SO_cz.md)
  : Municipal Election Data: Bublava (SO, CZ)
- [`Cernosice_PZ_cz`](https://localpolitics.github.io/lpanda/reference/Cernosice_PZ_cz.md)
  : Municipal Election Data: Cernosice (PZ, CZ)
- [`Dasnice_SO_cz`](https://localpolitics.github.io/lpanda/reference/Dasnice_SO_cz.md)
  : Municipal Election Data: Dasnice (SO, CZ)
- [`Doubice_DC_cz`](https://localpolitics.github.io/lpanda/reference/Doubice_DC_cz.md)
  : Municipal Election Data: Doubice (DC, CZ)
- [`Hradce_CB_cz`](https://localpolitics.github.io/lpanda/reference/Hradce_CB_cz.md)
  : Municipal Election Data: Hradce (CB, CZ)
- [`Jilove_DC_cz`](https://localpolitics.github.io/lpanda/reference/Jilove_DC_cz.md)
  : Municipal Election Data: Jilove (DC, CZ)
- [`Kamenna_CB_cz`](https://localpolitics.github.io/lpanda/reference/Kamenna_CB_cz.md)
  : Municipal Election Data: Kamenna (CB, CZ)
- [`Nebanice_CH_cz`](https://localpolitics.github.io/lpanda/reference/Nebanice_CH_cz.md)
  : Municipal Election Data: Nebanice (CH, CZ)
- [`Potucky_KV_cz`](https://localpolitics.github.io/lpanda/reference/Potucky_KV_cz.md)
  : Municipal Election Data: Potucky (KV, CZ)
- [`Prameny_CH_cz`](https://localpolitics.github.io/lpanda/reference/Prameny_CH_cz.md)
  : Municipal Election Data: Prameny (CH, CZ)
- [`Roztoky_PZ_cz`](https://localpolitics.github.io/lpanda/reference/Roztoky_PZ_cz.md)
  : Municipal Election Data: Roztoky (PZ, CZ)

### Examples for testing

- [`sample_data`](https://localpolitics.github.io/lpanda/reference/sample_data.md)
  : Simple Sample Dataset
- [`sample_different_varnames`](https://localpolitics.github.io/lpanda/reference/sample_different_varnames.md)
  : Sample Dataset with Some Different Variable Names
- [`sample_binary_values`](https://localpolitics.github.io/lpanda/reference/sample_binary_values.md)
  : Sample Dataset with Binary Values
- [`sample_no_continuity`](https://localpolitics.github.io/lpanda/reference/sample_no_continuity.md)
  : Sample Dataset Without Continuity Between Elections
- [`sample_no_switching`](https://localpolitics.github.io/lpanda/reference/sample_no_switching.md)
  : Sample Dataset Without Candidate Switching
- [`sample_no_pluralism`](https://localpolitics.github.io/lpanda/reference/sample_no_pluralism.md)
  : Sample Dataset Without Party Pluralism (only one candidate list per
  election)

## Package documentation

- [`lpanda`](https://localpolitics.github.io/lpanda/reference/lpanda-package.md)
  [`lpanda-package`](https://localpolitics.github.io/lpanda/reference/lpanda-package.md)
  : lpanda: Local Political Actor Network Diachronic Analysis Tools
